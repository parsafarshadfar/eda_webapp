"""Where results are saved, and how they are written.

One rule for the whole repository:

    **results are written to a ``results`` folder beside whatever you ran.**

* ``app.py`` (the Streamlit app) saves under ``<repo root>/results/profiling/``
  and ``<repo root>/results/drift/``;
* an example notebook saves under ``<notebook folder>/results/`` -- each
  notebook sets that once with :func:`set_output_dir`.

Every renderer and exporter accepts an explicit address as well, so one call can
always be pointed somewhere else without changing the default.

Databricks, DBFS and ADLS
-------------------------
An address is not required to be a local path. ``dbfs:/...``,
``abfss://container@account.dfs.core.windows.net/...`` and the other cloud
schemes are recognised and handled through ``dbutils.fs``, which means the
cluster's own credentials are used and no direct connection to the storage
account is opened from Python.

Two properties matter on a locked-down cluster, and both are guaranteed here:

* **nothing is streamed.** Every file is serialised to bytes in memory first and
  then written with a single call. No handle is kept open on the destination and
  no data is appended piece by piece -- the pattern ADLS through a corporate
  firewall usually refuses.
* **no pathlib on remote addresses.** ``Path("abfss://a/b")`` silently collapses
  the double slash, so remote addresses are handled as plain strings throughout.

The files written here are aggregated result tables and rendered figures, not
raw data, so holding one of them in memory before writing it is cheap.
"""
...
# an ordinary path as long as the whole file is written in one go, which is
# exactly what :func:`write_bytes` does.
_REMOTE_SCHEMES = (
    "dbfs:",
    "abfss://",
    "abfs://",
...
# Session-level override installed by set_output_dir(). ``None`` means "use the
# results/ folder next to this repository".
...
def default_output_dir() -> str:
    """``<repo root>/results/profiling``, or whatever was last set."""
    if _override is not None:
        return _override
    return join_address(str(project_root() / RESULTS_DIRNAME), PROJECT_NAME)


#: Readable alias: ``profiling.output_dir()`` answers "where do results go?".
output_dir = default_output_dir


def set_output_dir(address: str | os.PathLike[str] | None) -> str:
    """Point every ``save=True`` in this package at ``address``.

    A notebook calls this once, in its setup cell, so the rest of the notebook
    needs no addresses at all::

        profiling.set_output_dir(NOTEBOOK_DIR / "results")
        profiling.set_output_dir("abfss://data@account.dfs.core.windows.net/eda")
...
def parent_address(address: str | os.PathLike[str]) -> str:
    ...
    # Split the scheme off first so the "//" in "abfss://" is never mistaken
    # for a path separator, then drop the last component of what remains.
    trimmed = text.rstrip("/")
...
def with_suffix(address: str | os.PathLike[str], suffix: str) -> str:
    """Add ``suffix`` (for example ``".png"``) when the address has none."""
    text = _as_address(address)
    name = text.replace("\\", "/").rsplit("/", 1)[-1]
    return text if "." in name else f"{text}{suffix}"


def numbered_address(address: str | os.PathLike[str], page: int) -> str:
    """``chart.png`` -> ``chart_02.png``, used for multi-page output."""
    text = _as_address(address)
    head, separator, name = text.replace("\\", "/").rpartition("/")
    stem, dot, extension = name.rpartition(".")
    if not dot:
        stem, extension = name, "png"
    numbered = f"{stem}_{page:02d}.{extension}"
    combined = f"{head}{separator}{numbered}" if separator else numbered
    return _as_address(combined)
...
def write_bytes(address: str | os.PathLike[str], payload: bytes) -> str:
    ...
    ``dbutils.fs.cp``, so the driver performs a single upload and no incremental
    connection to the storage account is ever opened.
    """
    text = _as_address(address)
    if not is_remote(text):
        target = Path(text)
    ...
    handle, staged = tempfile.mkstemp(prefix="eda_upload_", suffix=suffix)
    os.close(handle)
    try:
...
def write_frame_csv(
    ...
    *,
    ...
    Deliberately not ``frame.to_csv(path)``: that keeps a handle open on the
